...
import { exitToHub } from '@/lib/exitToHub'
...
const DEFAULT_SEEN = new Set([0])
const SlideCtx = createContext<SlideContextValue>({
  currentSlide: 0,
  totalSlides: 0,
  seenSlides: DEFAULT_SEEN,
  goTo: () => {},
})
...
export function SlideEngine({
  ...
  const goTo = useCallback(
    (index: number) => {
      ...
      container.scrollTo({ left: clamped * container.clientWidth, behavior: prefersReduced ? 'instant' : 'smooth' })
    },
    [totalSlides],
  )
  ...
  const ctxValue = useMemo(
    () => ({ currentSlide: current, totalSlides, seenSlides: seen, goTo }),
    [current, totalSlides, seen, goTo],
  )
